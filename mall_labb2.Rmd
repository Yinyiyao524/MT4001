...
Wilcoxons teckenrangtest samt Wilcoxon-Mann_Whitneytest om effekten av molnsådd ökar nederbörden för respektive
...
samt Wilcoxon-Mann_Whitneytest för att få fram ett p-värde och se om det är större
...
1. t-test: om datan är kvantitativ, normalfördelad (eller tillräckligt stor sample-storlek för att approximera normalitet). Vi kan jämföra medelvärdet av nederbörd mellan dagar med och utan molnsådd för att testa om skillnaden är signifikant.
2. teckenrangtest: Om datan är kvantitativ men inte normalfördelad. Det är ett icke-parametriskt alternativ till parat t-test som jämför medianen av skillnader mellan olika grupper.
...
```
```{r}
wilcox.test(seed, nonseed, alternative = "two.sided")
```
...
2. 
2. Wilcoxons-Mann_Whitneytest eftersom denna på samma sätt som tecken-rangtestet
...
```{r}
wilcox.test(nonseed1, seed2, alternative = "two.sided", exact = FALSE)
wilcox.test(nonseed2, seed2, alternative = "two.sided",exact = FALSE)
```

```{r}
differ1 <- seed1-nonseed1
differ2 <- seed2-nonseed2
result1 <- wilcox.test(differ1, differ2, alternative = "greater") # The code you've written contains a logical issue. The Wilcoxon Mann-Whitney test is designed to compare two independent groups, not two sets of differences.
