...
```{r,fig.cap='Q-Q-plot: Utan molnsådd'}
...
skulle det vara rimligt att utföra t-test, vi undersöker därför grafiskt (Även om diffen är normalfördelad kan vi inte göra ett t-test här, eftersom våra stickprov inte är parvis beroende.)
...
Men vi ser här att $p>\alpha$ så vi kan därför inte förkasta nollhypotesen. ---(är du säker på at vi behöver göra ett t.test för diffen?)
...
```{r}
wilcox.test(seed1, nonseed1, alternative = "greater", paired = FALSE, conf.level = 0.95) 
```
Vi kan här se att $\alpha < p$ så vi kan ej förkasta nollhypotesen

```{r}
wilcox.test(seed2, nonseed2, alternative = "greater", paired = FALSE, conf.level = 0.95)
```
