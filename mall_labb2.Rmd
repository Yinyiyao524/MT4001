...
boxplot(differ, prob = TRUE, main = 'Boxplot: Differens')
```
...
Sammanfattande text
Histogram, boxplot och Q-Q-plottar visade att dataset inte var normalfördelade, vi väljer icke-parametrisk metod: teckenrangtest (Wilcoxon signed-rank test).
...
Vi kan här se att utseendet på vår Q-Q-plot ej efterliknar utseendet hos normalfördelad data
på grund av att Q-Q-plotens raka linje ej följs och en viss tendens till s-form kan synas.
...
qqline(nonseed1)
```
I detta fall tyder vår data ej heller på att den är normalfördelad eftersom den har
outliers och Q-Q-plotens raka linje ej följs. Vi fortsätter med en Q-Q-plot över
...
qqline(seed2)
```
Utseendet här tyder inte på normalfördelning på grund av att Q-Q-plotens raka linje
ej följs då avvikelserna är för stora. Vi fortsätter med Q-Q-plot över nederbörd
...
följas av vår datamängd. Vi fortsätter med att undersöka histogram över nederbörd
...
```{r}
result1 <- wilcox.test(seed1, nonseed1, alternative = "greater", paired = FALSE) # två grupper har olika mängd, därför är de oberoende (paired=FALSE)
print(result1)
```
Vi kan här se att $\alpha < p$ så vi kan ej förkasta nollhypotesen

```{r}
result2 <- wilcox.test(seed2, nonseed2, alternative = "greater", paired = FALSE)
print(result2)
```
